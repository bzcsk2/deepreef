...
## Phase 2：CLI 配置命令 ⏳ 待实现

### 计划任务

```text
1. deepreef config path     - 输出配置文件路径
2. deepreef config print    - 输出effective config（支持--json/--toml/--redact）
3. deepreef config validate - 校验配置文件
4. deepreef config init     - 初始化配置（支持--template）
5. deepreef config edit     - 打开编辑器编辑配置
6. deepreef config doctor   - 深度检查配置
```
