...
### 回归测试
- `phase-ab-acceptance.test.ts` 新增 2 个 `workflowId 集成` 测试：
  - `startWorkflow 使用传入的 workflowId` — sessionId 作为 workflowId，coordinator 读到同一 goal
  - `不传 workflowId 时仍生成随机 UUID` — 向后兼容

## 最终验证
- `bun test packages/core packages/tui packages/cli` — 1358 pass, 0 fail
- `bun run typecheck` — pass
- Git commits: `4850260`, `caa8981`, `a76b26e` — 21 files changed, 1649 insertions, 174 deletions
