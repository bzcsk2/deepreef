/**
 * Pure state machine for the question UI — adapted from OpenCode (MIT License).
 * Source: packages/opencode/src/cli/cmd/run/question.shared.ts
 *
 * Supports both single-question and multi-question flows.
 * Single questions submit immediately on selection.
...
export function questionSubmit(request: QuestionRequest, state: QuestionBodyState): { requestId: string; answers: string[][] } {
  return {
    requestId: request.id,
...
export function questionReject(request: QuestionRequest): { requestId: string } {
  return {
    requestId: request.id,
  }
}
